...
def InferOneImgMul(bin,img,image_name,save_path,n_class,class_names):
    ...
    for i in range(len(bbox_batch)):
        ...
        with open(os.path.join(save_path, 'result_txt', 'res_' + image_name[i] + '.txt'), 'w+', encoding='utf-8') as fid_res:
            ...
            for bbox in bboxes:
                bbox = bbox.reshape(-1, 2).astype(np.int)
                first_point = (int(bbox[0][0]), int(bbox[0][1]))
                img_show = cv2.drawContours(img_show, [bbox], -1, colors[str(int(classes[tag]))], 2)
                #将标签画在图片上
                cv2.putText(img_show, class_names[int(classes[tag])], first_point, cv2.FONT_HERSHEY_COMPLEX, fontScale=1, color=(255, 0, 0),thickness=1)
                bbox_str = [str(x) for x in bbox.reshape(-1)]
                bbox_str = ','.join(bbox_str) + '\n'
                fid_res.write(bbox_str)
                tag+=1
...
#         print(batch_img_names)
        ...
        for i in range(len(batch_imgs)):
            ...
            if 'n_class' in config['base'].keys():
                InferOneImgMul(test_bin, batch_imgs[i],batch_img_names[i], save_path,config['base']['n_class'],config['base']['class_names'])
            else:
        ...
        img = cv2.imread(path)
        if 'n_class' in config['base'].keys():
            InferOneImgMul(test_bin, [img], [image_name], save_path, config['base']['n_class'],config['base']['class_names'])
        else:
...
#     stream = open('./config/det_DB_resnet50.yaml', 'r', encoding='utf-8')
    ...
    parser.add_argument('--config', default='./config/det_DB_resnet50_mul.yaml',help='config file path')
    parser.add_argument('--model_path', nargs='?', type=str, default='weight/DB_20.pth.tar')
    parser.add_argument('--onnx_path', nargs='?', type=str, default=None)
    parser.add_argument('--trt_path', nargs='?', type=str, default=None)
    parser.add_argument('--img_path', nargs='?', type=str, default='img1/')
    parser.add_argument('--result_save_path', nargs='?', type=str, default='result')
    parser.add_argument('--max_size', nargs='?', type=int, default=736)
